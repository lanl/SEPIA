...
%    'thetaPrior'- structure including function name to add a joint prior
%                   of the theta values.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Process input arguments
  ...
  hierParams=[]; thetaPrior=[];
  parseAssignVarargs({'noCounter', 'clist','hierParams', ...
                      'step','initOnly','noInit','thetaPrior'});
    ...
    for iter=1:nmcmc
      ...
      for modi=1:numMods
        ...
        for varNum=1:length(svars)
          ...
          switch(C.var)
          case {'theta'} 
            %compute proposals for all thetas 
          thetaprop=subParams(modi).model.(C.var)(1:svarSize(varNum)) + ...
             (rand(1,svarSize(varNum))-.5).*subParams(modi).mcmc.(wvars{varNum})(1:svarSize(varNum)); 
         exclindex=[]; %indices to use block proposal, and exclude from single parameter proposal.    
          if ~isempty(thetaPrior)  
               thetaPrior=functhetaPrior(thetaprop,thetaPrior);
               exclindex=thetaPrior.parindex;
               if ~thetaPrior.out.yes
                   acc.(C.var)(1:svarSize(varNum))=0;
               else
                   C.index=thetaPrior.parindex;
                   C.val=thetaprop(thetaPrior.parindex);
                   subParams=mcmcEval(subParams,modi,C,clist);
                   acc.(C.var)(C.index)=subParams(modi).model.acc;
               end
           %single parameter proposal
             % if (isempty(thetaPrior))    
              for k=1:svarSize(varNum)
                  if isempty(find(exclindex==k,1))
                      C.index=k;
...
%       ~isreal(C.val));
    if (any(C.val<priors.(C.var).bLower) || ...
      any(priors.(C.var).bUpper<C.val) || ...
      ~all(isreal(C.val)))
    % if a theta and cat, ignore bounds and 
    if ~ ( strcmp(C.var,'theta') && ... 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
function thetaPrior=functhetaPrior(thetaprop,thetaPrior)

    %cd(thetaPrior.funcfold); %change directory to folder of prior function
    %transform proposed parameters to original scale 
    xmin = thetaPrior.xmin;
    xrange = thetaPrior.xrange;    
    thetapropraw=bsxfun(@times, thetaprop, xrange); 
    thetapropraw=bsxfun(@plus, thetapropraw, xmin);
    
    %call function to get  
    thetaPrior.out.yes=thetaPrior.func(thetapropraw,thetaPrior);
    %cd(thetaPrior.codefold); %change directory to folder for GPMSA

end

function thetaPrior=functhetaPriorfull(C,thetaPrior,subParams)

     thetaprop=subParams(modi).model.(C.var)(1:svarSize(varNum)) + ...
            (rand(1,svarSize(varNum))-.5).*subParams(modi).mcmc.(wvars{varNum})(1:svarSize(varNum)); 
       thetaPrior=functhetaPrior(thetaprop,thetaPrior);
       
       if (any(thetaprop<priors.(C.var).bLower)|| any(thetaprop>priors.(C.var).bUpper)...
          || ~all(isreal(C.val)) || ~thetaPrior.out.yes)
               acc.(C.var)(1:svarSize(varNum))=0;
       end
    if (isempty(thetaPrior)) || (thetaPrior.out.yes)    
      for k=1:svarSize(varNum)
          C.index=k;
              %C.val=subParams(modi).model.(C.var)(k) + ...
               %   (rand(1)-.5)*subParams(modi).mcmc.(wvars{varNum})(k);
