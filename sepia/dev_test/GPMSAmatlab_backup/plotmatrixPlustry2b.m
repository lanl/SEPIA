function plotmatrixPlustry2b(data,varargin)
     ...
     for jj=1:length(zPAx)
       ...
       if (ucontour) && (ii~=jj)
         ...
         ksContour(data(pmsamp,[jj ii]),[0.025 0.25 0.5 0.75 0.975], ...
               'ngrid',25,'ax',[xmin(jj) xmax(jj) xmin(ii) xmax(ii)], ...
               'lam',lam,'flip',false,'truepar',[truepar(jj),truepar(ii)])
       end
       axj=[xmin(jj) xmax(jj)];
       axis(zAX(ii,jj),[axj axi]);
       set(zAX(ii,jj),'YDir','normal')
    set(zAX(ii,jj),'FontSize',fsize1);
       ...
       if exist('labels')
         if jj==1; ylabel(zAX(ii,jj),labels{ii},'FontSize',fsize2,'FontWeight','bold'); end
         if ii==1; title(zAX(ii,jj),labels{jj},'FontSize',fsize3); end
       end
       hold(zAX(ii,jj),'on');  
