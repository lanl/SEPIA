...
parseAssignVarargs({'clist','hierParams','thetaPrior'});
...
[params hierParams]=gpmmcmcnew(params,0,'initOnly',1,'clist',clist,...
                            'hierParams',hierParams,'thetaPrior',thetaPrior);
...
for burn=1:nBurn
  for lev=1:nLev
    ...
    [params hierParams]=gpmmcmcnew(params,1,'noInit',1,'noCounter',1,...
                         'step',1,'clist',clist,'hierParams',hierParams,'thetaPrior',thetaPrior);
  end
end
counter('end');
...
for ii=1:numMods
  for varNum=1:length(params(ii).mcmc.svars)
    ...
    if ~isempty(thetaPrior) && strcmp(wvar,'thetawidth') 
        params(ii).mcmc.(wvar)(thetaPrior.parindex)=params(ii).mcmc.(wvar)(thetaPrior.parindex)/length(thetaPrior.parindex);
    end   
  end
end
