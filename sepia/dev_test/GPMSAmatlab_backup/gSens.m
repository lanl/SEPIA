...
% process input arguments
 ...
 %correct for separable covariances
 if ~isempty(pout.data.ztSep)
    xtemp=pout.data.zt;
 else
     xtemp=pout.simData.x;
 end
 

 for ii=1:pu
    betaU=BetaU(:,ii0+(ii-1)*nv); lamUz=LamUz(:,ii); lamWs=LamWs(:,ii);
    sa(ii)=componentSens(xtemp(:,ii0),pout.data.w(:,ii),betaU,...
...
function C2=calc2(x,xdist,m,rg,beta,diff)
 ...
 for ii=1:m-1
  ...
  for jj=ii+1:m
   kk=kk+1;
   mp=(x(ii,:)+x(jj,:))/2; 
   %di=find(xdist.indi==ii & xdist.indj==jj);
   di=kk;
