...
class SepDesSetupLogLik(unittest.TestCase):
    ...
    def test_full_setup_LL_pred(self):
        ...
        print('Running MCMC on Calibration model in Sep pred mode, regular design')
        np.random.seed(42)
        t1=time()
        cmod.do_mcmc(10)
        ...
        print('Running MCMC on Calibration model in Sep pred mode, separable design')
        np.random.seed(42)
        t1=time()
        kcmod.do_mcmc(10)
        print('sampling time %f'%(time()-t1))
    ...
    def test_full_setup_LL_pred_scalar_out(self):
        # Set up and check calibration model
        x_obs = np.ones((3, 2)) * np.array([0.5, 0.75, 0.25]).reshape((-1, 1))
        y_obs = np.array([-0.1,-0.2,0.1]).reshape((-1,1))
        ...
        y_sim=self.y_sim[:,0].reshape((-1,1))

        y_sim_std = (y_sim - np.mean(y_sim, axis=0).reshape(1, -1)) / \
                    np.std(y_sim, axis=0, ddof=1).reshape(1, -1)

        dc = SepiaData(x_sim=x_sim_cal, t_sim=t_sim_cal, y_sim=y_sim_std, x_obs=x_obs, y_obs=y_obs)
        dc.create_K_basis(K=np.eye(2))
        dc.create_D_basis(D_sim=np.eye(2), D_obs=np.eye(2))
        dc.transform_xt(x_notrans=True, t_notrans=True)
        dc.standardize_y(y_mean=0, y_sd=1)
        print(dc)
        cmod = SepiaModel(dc)
        print('Calibration model LL=%f\n' % compute_log_lik(cmod))

        kdc = SepiaData(xt_sim_sep=xt_sim_sep, y_sim=y_sim_std, x_obs=x_obs, y_obs=y_obs)
        kdc.create_K_basis(K=np.eye(2))
        kdc.create_D_basis(D_sim=np.eye(2), D_obs=np.eye(2))
        kdc.transform_xt(x_notrans=True, t_notrans=True)
        kdc.standardize_y(y_mean=0, y_sd=1)
    ...
    def test_full_setup_LL_pred_noD(self):
        # Set up and check calibration model
        x_obs=np.ones((3,2)) * np.array([0.5,0.75,0.25]).reshape((-1,1))
        y_obs=np.block([[-0.1,0.1],[-0.2,0.3],[0.1,0]])
    ...
    def test_full_setup_LL_pred_scalar_out_noD(self):
        # Set up and check calibration model
        x_obs = np.ones((3, 2)) * np.array([0.5, 0.75, 0.25]).reshape((-1, 1))
        y_obs = np.array([-0.1,-0.2,0.1]).reshape((-1,1))
